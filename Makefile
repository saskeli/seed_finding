...
CPPFLAGS				+= -DMAX_GAP=$(MAX_GAP) -DHWY_IS_DEBUG_BUILD=0 $(INCLUDE)
...
TEST_CPPFLAGS			+= -DGTEST_ON -DHWY_IS_DEBUG_BUILD=0 $(INCLUDE) -isystem $(GTEST_DIR)/googletest/include -isystem $(RAPIDCHECK_DIR)/include -isystem $(RAPIDCHECK_DIR)/extras/gtest/include
...
include/version.hpp: .git
	...
	printf "// This file is automatically generated by Makefile.\n#include <string_view>\nnamespace sf {\n  constexpr static std::string_view const version{\"%s\"};\n}\n" "$${VERSION_STR}" > "$@"

correlations.o: include/version.hpp
seed_finder.o: include/version.hpp
