...
class lossy_hash_map {
 ...
 private:
  ...
  hash_type hash_;
  std::size_t size_{};
  uint16_t max_iterations_{UINT16_MAX};
 ...
 public:
  ...
  explicit lossy_hash_map(allocate_tag) : elements_(MOD) {}
  ...
  value_type* insert_or_find(key_type key);
  void clear();
...
std::size_t lossy_hash_map<t_key, t_value, t_hash>::find_(key_type key) const {
  ...
  auto idx{hh % MOD};
  for (uint16_t ii{}; uint64_t(elements_[idx].key) &&
                      elements_[idx].key != key && ii < max_iterations_;
       ++ii) [[unlikely]] {
    idx += STEP;
    idx -= idx >= MOD ? MOD : 0;
  }
...
template <UInt64Constructible t_key, typename t_value, typename t_hash>
auto lossy_hash_map<t_key, t_value, t_hash>::find(key_type key) const
    -> value_type const* {
  auto& el{elements_[find_(key)]};
  if (el.key != key) return nullptr;
  return &el.value;
...
template <UInt64Constructible t_key, typename t_value, typename t_hash>
auto lossy_hash_map<t_key, t_value, t_hash>::insert_or_find(key_type key)
    -> value_type* {
  auto& el{elements_[find_(key)]};
