...
// The purpose of this class template is to make it easy to align strings to a
// buffer that is aligned to some number of bytes. An option would have been to
// use a custom allocator for std::string but unfortunately the rules concerning
// accessing values of some type that is not char or std::byte are somewhat
// unclear to me.
...
class string_buffer {
 ...
 public:
  ...
  std::span<char> to_span() {
    return {reinterpret_cast<char *>(data_.data()), size_};
  }

  std::span<char const> to_span() const {
    return {reinterpret_cast<char const *>(data_.data()), size_};
  }
  ...
  char operator[](std::size_t idx) const {
    return 0xff &
           (data_[idx / sizeof(t_type)] >> (idx % sizeof(t_type) * CHAR_BIT));
  }
  ...
  string_buffer &operator<<=(uint64_t const amt) {
    shift_left(amt);
    return *this;
  }
...
void string_buffer<t_type>::shift_left(uint64_t const amt) {
  // Deletes characters from the left end, does not reset the characters at the
  // right end.
  if (size_ <= amt) {
    size_ = 0;
    return;
...
void string_buffer<t_type>::shift_right(uint64_t const amt) {
  // Deletes characters from the right end, does not reset the characters at the
  // left end.
  if (size_ <= amt) {
    size_ = 0;
    return;
