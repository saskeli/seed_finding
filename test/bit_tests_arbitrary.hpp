...
namespace rc {
	...
	struct Arbitrary <short_dna_string>
	{
		...
		{
			return gen::mapcat(gen::inRange <std::size_t>(1, 9), [](std::size_t const length){
				return gen::construct <short_dna_string>(
					gen::container <std::string>(length, gen::arbitrary <nucleotide>())
				);
			});
		}
	};
...
namespace sf {
	...
	RC_GTEST_PROP(bit_arbitrary, ReadMultipleCharacters, (short_dna_string const &dna_str)) {
